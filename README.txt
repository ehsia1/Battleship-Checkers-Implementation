...
- FOR THE CHECKERS TESTER:
	-> when prompted for input, enter the following in the specified order and hit enter after each one: "23TR", "23TR","72TL", "12TR". The reason for
	this is because we prompt for the double jumps in the checkers.cpp file, so when the tester is run, it asks for this.
